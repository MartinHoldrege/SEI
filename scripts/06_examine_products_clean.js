...
// the paramaters (RCP, run, etc, need to be set in this script)
var d = require("users/mholdrege/SEI:scripts/05_lyrs_for_apps.js");
...
map.addLayer(d.climCur.select('MAT'), MATvis, 'MAT (historical, interpolated)', false);
...
for (var i = 0; i < bandsRed.length; i++) {
  var b = bandsRed[i];
  map.addLayer(d.climDeltaRed.select('MAP_' + b), deltaMAPvis, 'delta MAP (' + b + ', interpolated)', false);
}

// MAT
for (var i = 0; i < bandsRed.length; i++) {
  var b = bandsRed[i];
  map.addLayer(d.climDeltaRed.select('MAT_' + b), deltaMATvis, 'delta MAT (' + b + ', interpolated)', false);
}
...
map.addLayer(d.qPropMean, rgbViz, 'RGB (delta Q attribution)' + rgbLab, false);
...
for (var j = 0; j < diffBands.length; j++) {
  ...
  map.addLayer(d.diffRed2.select(band + '_low').sldStyle(sldRampDiff1), {}, 'delta ' + namesBands[j] + ' (low, across GCMs)', false);
  map.addLayer(d.diffRed2.select(band + '_high').sldStyle(sldRampDiff1), {}, 'delta ' + namesBands[j] + ' (high, across GCMs)', false);
  
  // median is already pre-computed for Q5s
  if (diffBands == 'Q5s') {
    var medianLyr = d.p.select('p1_diffQ5sMed');
  } else {
    var medianLyr = d.diffRed2.select(band + '_median');
  }
...
var panelDescript = ui.Label({
  value: 'STEPWAT simulation settings: ' + d.root + ' (' + d.RCP + ', ' + d.epoch + ')' + ' (' + d.versionFull + ')',
  style: {
    fontSize: '12px',
    margin: '0 0 4px 0',
    padding: '0'
