...
sw_prob2 <- bind_cols(sw_prob1, create_rast_info(sw_prob1$layer, into = into)) %>% 
  select(-layer) %>% 
  mutate(PFT = PFTlookup0[as.character(PFT)])
...
map(PFTabbr, function(pft) {
  ...
  cdfoc <- cov_cdf1[[cols[lyr]]]
  
  lyrf <- info2 %>% # current stepwat layer
    filter(PFT == pft,
           RCP != 'Current') %>% 
    pull(id)
  stopifnot(length(lyrf) == 1)
  x_mf <- sw_cov_df1[[lyrf]]
