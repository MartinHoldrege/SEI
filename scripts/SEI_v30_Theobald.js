...
for (var ii = 2022; ii<2023; ii++) {   // 2022 is  available  in v3.0 of RAP
  

  var yearEnd = ii  // this value is changed to make multi-year runs, e.g., 2017-2020 would= 2020
  var yearStart = yearEnd - 3 // inclusive, so if -3 then 2017-2020, inclusive
  
  var connectivityRun = false
  var onlySEIQ5 = false
  ...
  var version = '3'   // 2=v2 of RAP, 3=v3 of RAP
  var runMTeasements = false   // for special run for Geoff Bedrosian, MT easements
  var sensitivityRun = false
  ...
  if (runMTeasements) {   //remember to change export assetId name
    Map.addLayer(H,{},'H before...',true)
    var MTeasements = ee.FeatureCollection('projects/ee-geoffbedrosian/assets/NGP_easements')
    var MTeasementsToRemove = ee.Image(0.0).paint(MTeasements,1.0)
    Map.addLayer(MTeasementsToRemove.subtract(H),{min:0,max:1.0},'H and H modified by easements NEQ')
  
    var H = H.max(MTeasementsToRemove)
    print('Removed SEI from MT easements...')
    Map.addLayer(H,{},'H after...',true)
    Map.addLayer(MTeasements,{},'MT easements',true)
  }
  ...
  if (version == 2) {
    ...
    var srcmapSage = "users/DavidTheobald8/USGS/RCMAP/rcmap_sagebrush_"
    var sNameAnnual = 'AFGC'
    var sNamePerrenial = 'PFGC'
    var sNameShrubland = 'SHR'
    var sNameTree = 'TREE'
    var sNameSage = 'b1'
    var sversionFN = '11'
    ...
    var srcmapSage = "USGS/NLCD_RELEASES/2019_REL/RCMAP/V5/COVER/"	
    var sNameAnnual = 'AFG'
    var sNamePerrenial = 'PFG'
    var sNameShrubland = 'SHR'
    var sNameTree = 'TRE'
    var sNameSage = 'rangeland_sagebrush'
    var sversionFN = '30'
    print('Running version 3')
  }
  else {
    print( 'Version is not supported: ', version)
  }
  ...
  */
  ...
  var wildfires = ee.FeatureCollection('users/DavidTheobald8/WFIGS/Interagency_Fire_Perimeter_History_20230828')  // previous v2 used: Interagency_Fire_Perimeter_History
  ...
  var rapAnnualG = rap.select(sNameAnnual) // AFG
    .multiply(tundra) // mask out tundra grass/shrub
  var rapPerennialG = rap.select(sNamePerrenial) // PFG 
    .multiply(tundra) // mask out tundra grass/shrub
  var rapShrub = rap.select(sNameShrubland)
    .multiply(tundra) // mask out tundra grass/shrub
  var rapTree = rap.select(sNameTree)
  ...
  for (var i=yearStart; i<=yearEnd; i++) {
    ...
    var rcmapSage = ee.Image(srcmapSage + yearSage).select(sNameSage)
   ...
   */
  var nlcdSage560m = nlcdSage.reduceNeighborhood(ee.Reducer.mean(),ee.Kernel.gaussian(560,560 * 0.5,'meters'))
    .divide(100.0)
    .unmask(0.0);
  var rapAnnualG560m = rapAnnualG.reduceNeighborhood(ee.Reducer.mean(),ee.Kernel.gaussian(560,560 * 0.5,'meters'))
    .divide(100.0)
    .unmask(0.0);
  var rapPerennialG560m = rapPerennialG.reduceNeighborhood(ee.Reducer.mean(),ee.Kernel.gaussian(560,560 * 0.5,'meters'))
    .divide(100.0)
    .unmask(0.0);
  var H560m = H.reduceNeighborhood(ee.Reducer.mean(),ee.Kernel.gaussian(560,560 * 0.5,'meters'))
    .unmask(0.0);
  var rapTree560m = rapTree.reduceNeighborhood(ee.Reducer.mean(),ee.Kernel.gaussian(560,560 * 0.5,'meters'))
    .divide(100.0)
    .unmask(0.0);
  ...
  if (onlySEIQ5 ) {
    ...
    assetId: 'users/DavidTheobald8/WAFWA/v' + sversionFN + '/SEI_Q5_v' + sversionFN + '_' + yearStart + '_' + yearEnd + '_' + resolution + '_20230317',
   ...
   // crsTransform: proj.transform
    ...
    var imageVisQ5sc = {"opacity":1,"min":1,"palette":["e7ed8b","23b608","107a0e","082b08"]}
    ...
    */
    ...
    Export.image.toAsset({ 
      ...
      assetId: 'users/DavidTheobald8/WAFWA/v' + sversionFN + '/SEI_v' + sversionFN + '_' + yearStart + '_' + yearEnd + '_' + resolution + '_20230828',
      description: 'SEI' + yearStart + '_' + yearEnd + '_' + resolution,
      maxPixels: 1e13, scale: resolution, region: region,
      crs: 'EPSG:4326',    // set to WGS84, decimal degrees
      //crsTransform: proj.transform
    ...
    /*
    Export.image.toAsset({ 
      image: WAFWAoutputs.select('Q5sc3'), //single image with multiple bands
      assetId: 'users/DavidTheobald8/WAFWA/v' + sversionFN + '/SEIv_Q5sc3_v' + sversionFN + '_' + yearStart + '_' + yearEnd + '_' + resolution + '_20230626',
      description: 'SEI_SEIv_Q5sc3_' + yearStart + '_' + yearEnd + '_' + resolution,
      maxPixels: 1e13, scale: resolution, region: region,
      crs: 'EPSG:4326' ,   // set to WGS84, decimal degrees
     // crsTransform: proj.transform
    })
    */
    ...
    if (sensitivity != 1.0) {
      Export.image.toAsset({ 
        ...
        assetId: 'users/DavidTheobald8/WAFWA/v' + sversionFN + '/s1_Q5s_SEI_v' + sversionFN + '_' + yearStart + '_' + yearEnd + '_' + resolution + '_20211228',
